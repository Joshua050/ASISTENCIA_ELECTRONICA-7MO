...
String Preceptor[8][2] = {
  {"Bianca Arzamendia", "F3 9A C1 AB"},
  {"Julian Gonzalez", "39 AB 8C 65"},
};
...
LiquidLine linea2(1, 2, "Turno");
LiquidLine linea6(1, 3, "CURSO");
...
void SERVOSD() {
  ...
  /*if (file_GEN == 0) {
    startime2 = millis();
    while ((millis() - startime2) < 4000 )// verifica sd y tiempo de espera
    {

      lcd.setCursor(0, 0);
      lcd.print("Primero genere");
      lcd.setCursor(0, 1);
      lcd.print("el archivo de la");
      lcd.setCursor(0, 2);
      lcd.print("asistencia porfavor");
    }
    lcd.clear();
  ...
  if (digitalRead(fcarrera2) == LOW) {//puerta cerrada
...
void veriftime() {
  if (manana == 1) {
    if (hora == 7 && minut > 35 and minut < 50) { //los valores son de prueba
      ESTADO = 2;
    }
  } else if (tarde == 1) {
    if (hora == 17 && minut > 35 and minut < 50) { //los valores son de prueba
      ESTADO = 2;
    }
  }
}
...
void printDatosMatriz() {
  for (byte row = 0; row < numRows; row++) {
    Serial.println(dataArray[row][0]);
  }
